...
function Questions() {
	...
	const handleCategoriesClick = (idCategory) => {
		...
		// };
		fetch(categoriespreferencesUrl, {
			method: 'PUT',
			headers: {
				'Accept': 'application/json',
				'Content-Type': 'application/json'
	...
	return (
		<div id='questionsdiv'>
			<div className='preferencesquestion'>
				<div>
					<span className="close cancel-btn" onClick={(e) =>
						navi('/dashboard', {
							state: {
								authorized: true,
								userObject: user
					...
					<div className='answer-section' id='ansSecCtry'>
						{countries &&
							countries.map((countryOption) => (
								<button key={countryOption.id} id={countryOption.id} onClick={() => handleCountriesClick(countryOption.id)}>{countryOption.countryName}</button>
							))}
					</div>
				) :
					(<div className='answer-section' id='ansSecCtg'>
						{categories &&
							categories.map((categoryOption) => (
								<button key={categoryOption.id} id={categoryOption.id} onClick={() => handleCategoriesClick(categoryOption.id)}>{categoryOption.name}</button>
							))}
					</div>)}
			</div>
		</div>
