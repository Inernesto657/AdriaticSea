 ...
 */
  ...
  useEffect(() => {
    ...
    const getData = () => {
      ...
      fetch(countriesPreferencesUrl).then((response) => response.json())
        .then((data) => {
          console.log(data);
          setCountryPreferences(data)
        });
      fetch(categoriesPreferencesUrl).then((response) => response.json())
        .then((data) => {
          console.log(data);
  ...
  const loopCountry = (countryOptions, countryPreferences) => {
    if (countryPreferences.country == countryOptions.id) {
    ...
    <div>
      ...
      <div className="news-feed-container">
        {newsFeeds && newsFeeds.map((newsFeed) => {
          ...
          <div className='news-feed-box no-record'><h3>No Record For This Search was found</h3></div>
        }
      </div>
    </div>
  );
